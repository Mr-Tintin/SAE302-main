...
class EcranPrincipal(QMainWindow):
    ...
    def consoletraitement(self):
        while True:
            data = client.recv(1024).decode()
            data1 = self.data.split(".info")
            for i in data1:
                if i!= "":data1 = str(i)
            if data > data1:
                self.console.append(data1)
            else:
                print("marchepas")
...
if __name__ == '__main__':
    ...
    thread = threading.Thread(target = EcranPrincipal().consoletraitement())
    thread.start()
